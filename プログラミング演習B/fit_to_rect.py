...
def fit_to_rect(img, size=(100, 100)):
    (w, h) = img.get_size()                   # 元の画像の大きさを取得する。
    (size_width, size_height) = size# 指定のスケールの大きさを取得する。
    if w == 0 or h == 0 or size_width <= 0 or size_height <= 0:
        return img
    scale =  min(size_width / w,size_height / h)  # 倍率を求める。
...
want_size = (100, 100)               # スケール後の画像の大きさ
if len(sys.argv) == 4:               # 大きさがコマンドライン引数で指定されていれば
    want_size = (int(sys.argv[2]), int(sys.argv[3])) # その大きさを用いる。
img = fit_to_rect(orig, want_size) # 指定の大きさにスケールした画像を得る。
